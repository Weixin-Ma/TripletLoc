...
struct RegistrationSolution {
  ...
  double mcq_time; //added by melo
  double solve_time; //added by melo
...
public:
  ...
  /**
   * Build consistency graoh and find max clique, custom implementation by weixin ma for TripletLoc
   * @return the max clique
   */
  std::pair<std::vector<int>, double> solveMcq(const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
                                               const Eigen::Matrix<double, 3, Eigen::Dynamic>& dst);
