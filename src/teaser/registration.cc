 ...
 */
#define NDEBUG     //set tesaser not to print debug information, added by melo

#include "./../include/teaser/registration.h"
...
#include "./../include/teaser/utils.h"
#include "./../include/teaser/graph.h"
#include "./../include/teaser/macros.h"
...
void teaser::TLSScaleSolver::solveForScale(const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
                                           ...
                                           double* scale,
                                           Eigen::Matrix<bool, 1, Eigen::Dynamic>* inliers) {   //这个需要估计scale

  Eigen::Matrix<double, 1, Eigen::Dynamic> v1_dist =
      src.array().square().colwise().sum().array().sqrt();            //这里求的是每个TIMS中对应的两个3D点的距离(local observation)，也即在local 挂测中，第i个观测和组成某个TIMS的第看个观测之间的三维点
  Eigen::Matrix<double, 1, Eigen::Dynamic> v2_dist =
      dst.array().square().colwise().sum().array().sqrt();            //这里求的是每个TIMS中对应的两个3D点的距离(glboal map)，也即在local 挂测中，第i个观测和组成某个TIMS的第看个观测之间的三维点

  Eigen::Matrix<double, 1, Eigen::Dynamic> raw_scales = v2_dist.array() / v1_dist.array();    //基于每个TIM计算得到的一系列sacle
  double beta = 2 * noise_bound_ * sqrt(cbar2_);                                              //这里应该是对应公式（9）左右两侧的误差项，正好基于(7)式，等于beta噪声bound的2倍
  Eigen::Matrix<double, 1, Eigen::Dynamic> alphas = beta * v1_dist.cwiseInverse();            //这个是计算的（TRIM）式子下面对应的alpha_{ij} 
...
void teaser::ScaleInliersSelector::solveForScale(
    ...
    Eigen::Matrix<bool, 1, Eigen::Dynamic>* inliers) {              //这个不需要估计scale
  ...
  *inliers = (v1_dist.array() - v2_dist.array()).array().abs() <= beta;       //基于TIM过滤外点，也即 dis(A-B)-dis(A'-B') 距离小于beta, 也即两倍误差界，认为是外点
}
...
teaser::RobustRegistrationSolver::computeTIMs(const Eigen::Matrix<double, 3, Eigen::Dynamic>& v,
  ...
  auto N = v.cols();      //added by melo, 这里N是表示有多少个matches
  Eigen::Matrix<double, 3, Eigen::Dynamic> vtilde(3, N * (N - 1) / 2);     //N * (N - 1) / 2 = (N-1) + ... + 0，也即总共有多少个TIMS
...
#pragma omp parallel for default(none) shared(N, v, vtilde, map)
  for (size_t i = 0; i < N - 1; i++) {
    ...
    Eigen::Matrix<double, 3, Eigen::Dynamic> temp = v - m * Eigen::MatrixXd::Ones(1, N);       //当前第i个观测对应的所有N个TIMS，自己和自己则为0（一个TIMS 需要local 或者 global中的连个vertex才能构建）

    // concatenate to the end of the tilde vector
    vtilde.middleCols(segment_start_idx, segment_cols) = temp.rightCols(segment_cols);        //将当前第i个观测对应的所有N个TIMS拼接到最终结果上

    // populate the index map
    Eigen::Matrix<int, 2, Eigen::Dynamic> map_addition(2, N);                                 //构建映射，当前第i个观测对应的所有N个TIMS中各自对应到输入观测v中的两个观测的id
    for (size_t j = 0; j < N; ++j) {
      map_addition(0, j) = i;
      map_addition(1, j) = j;
    }
    map->middleCols(segment_start_idx, segment_cols) = map_addition.rightCols(segment_cols);   //映射拼接
  }
...
teaser::RobustRegistrationSolver::solve(const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
  ...
  const auto t_mcq_start = std::chrono::high_resolution_clock::now();
  if (params_.inlier_selection_mode != INLIER_SELECTION_MODE::NONE) {
    ...
    // dst_tim_map_
    inlier_graph_.populateVertices(src.cols());     //只是对inlier_graph_进行resize，根据现有的src观测数量
    for (size_t i = 0; i < scale_inliers_mask_.cols(); ++i) {   //基于scale_inliers_mask_（这里这个名字起的感觉不是很好==）,满足consistent的 association 
      if (scale_inliers_mask_(0, i)) {
        inlier_graph_.addEdge(src_tims_map_(0, i), src_tims_map_(1, i));       //只有满足consistent 的 association才会连接起来，不满足的就不连接，事实上这个graph中包含的association就是论文D.Boosting Performance: Max Clique Inlier Selection中的G'(V'，E').
      }
    }
    ...
    max_clique_ = clique_solver.findMaxClique(inlier_graph_);     //further setting? and check? //added by melo
    std::sort(max_clique_.begin(), max_clique_.end());
    TEASER_DEBUG_INFO_MSG("Max Clique of scale estimation inliers: ");
    //std::cout<<"by melo: max clique size="<<max_clique_.size()<<std::endl;
#ifndef NDEBUG
  ...
  const auto t_mcq_end = std::chrono::high_resolution_clock::now();
  const std::chrono::duration<double, std::milli> diff = t_mcq_end - t_mcq_start;
  double ms_mcq = diff.count(); 
  solution_.mcq_time = ms_mcq;
  ...
  const std::chrono::duration<double, std::milli> diff1 = t_solve_end - t_solve_start;
  double ms_solve = diff.count(); 
  solution_.solve_time = ms_mcq;
...
double teaser::RobustRegistrationSolver::solveForScale(
  ...
  scale_inliers_mask_.resize(1, v1.cols());                    //也即原文中基于scale计算得到的inliers
  scale_solver_->solveForScale(v1, v2, &(solution_.scale), &scale_inliers_mask_);   //这个scale_inliers_mask_ 其实就是TIM的内点，过滤掉一部分abs ( dis(A-B)-dis(A'-B') ) 大于2倍noisebound的 association
...
//added by melo, only perform max clique selection
std::pair<std::vector<int>, double> teaser::RobustRegistrationSolver::solveMcq(const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
                                                                               const Eigen::Matrix<double, 3, Eigen::Dynamic>& dst)
{
  const auto t_mcq_start = std::chrono::high_resolution_clock::now();
  std::pair<std::vector<int>, double> max_clique_and_time;
  ...
  max_clique_and_time.first = max_clique_;    //using this to estimate pose, need to further check its size
  max_clique_and_time.second = ms_mcq;

  return max_clique_and_time;
}
